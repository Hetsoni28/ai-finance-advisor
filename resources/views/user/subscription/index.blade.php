...
@php
    ...
    // ✅ REAL DATA from SubscriptionController
    $currentPlan = $currentPlanSlug ?? 'starter';
    $currentBilling = strtolower($user->billing_cycle ?? 'monthly');
    $remainingDays = $subscription ? $subscription->remaining_days : null;
    $remainingText = $subscription ? $subscription->remaining_text : 'Free Tier — No Expiry';
    $endDate = $subscription && $subscription->end_date ? $subscription->end_date->format('M d, Y') : null;
    
    // Simulated Usage Data (Cosmetic — kept from original UI)
    $usage = [
        'ai_queries' => ['used' => 840, 'total' => 1000, 'percent' => 84],
        'api_calls'  => ['used' => 12500, 'total' => 100000, 'percent' => 12.5],
    ];

    // Simulated Payment Method (Cosmetic)
    $paymentMethod = [
        'brand' => 'visa',
        'last4' => '4242',
        'exp'   => '12/28',
    ...
    // ✅ REAL Invoice History from DB
    $dbInvoices = \App\Models\UserSubscription::with('plan')
    ...
    $invoices = $dbInvoices->map(fn($s) => [
        'id'     => $s->payment_id,
        'date'   => $s->created_at->format('M d, Y'),
        'amount' => '₹' . number_format((float)$s->amount_paid, 2),
        'status' => $s->status === 'active' ? 'Paid' : ucfirst($s->status),
...
<div class="min-h-screen bg-[#f8fafc] pb-32 font-sans selection:bg-indigo-500 selection:text-white relative overflow-hidden">
    ...
    <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10 space-y-12">
        ...
        @if(session('error'))
            <div id="sysError" class="bg-rose-50 border border-rose-200 rounded-[1.5rem] p-4 flex items-center justify-between shadow-sm max-w-3xl mx-auto animate-fade-in-up">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-[12px] bg-rose-500 text-white flex items-center justify-center shrink-0 shadow-md shadow-rose-500/30 border border-rose-400"><i class="fa-solid fa-triangle-exclamation text-sm"></i></div>
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-rose-600 mb-0.5">System Alert</p>
                        <p class="text-sm font-bold text-slate-700 leading-tight">{{ session('error') }}</p>
                    </div>
                </div>
                <button onclick="document.getElementById('sysError').remove()" class="text-rose-600 hover:bg-rose-100 w-8 h-8 rounded-lg flex items-center justify-center transition-colors focus:outline-none"><i class="fa-solid fa-xmark"></i></button>
            </div>
        @endif
                ...
                <div class="lg:col-span-4 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-slate-100 pb-8 lg:pb-0 lg:pr-8">
                    <div>
                        ...
                        <div class="flex items-end gap-3 mb-4">
                            <h2 class="text-4xl font-black text-slate-900 capitalize tracking-tight">{{ $currentPlan === 'free' ? 'Starter' : ($currentPlan === 'pro' ? 'Pro' : ($currentPlan === 'premium' ? 'Enterprise' : ucfirst($currentPlan))) }}</h2>
                        ...
                        <p class="text-sm text-slate-500 font-medium leading-relaxed">
                            ...
                            @else
                                Your node is fully synchronized. Next billing cycle resolves on <strong class="text-slate-700">{{ $endDate ?? now()->addDays(14)->format('M d, Y') }}</strong>.
                            @endif
                        </p>
                        @if($subscription && $currentPlan !== 'free')
                        <div class="mt-4 flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full {{ $remainingDays <= 5 ? 'bg-rose-500 animate-pulse' : 'bg-emerald-500' }}"></div>
                            <span class="text-xs font-black {{ $remainingDays <= 5 ? 'text-rose-600' : 'text-emerald-600' }} uppercase tracking-widest">{{ $remainingText }}</span>
                        </div>
                        @endif
                    </div>
        ...
        <div class="pt-8 animate-fade-in-up" style="animation-delay: 200ms;">
            ...
            <div class="grid lg:grid-cols-3 gap-8 items-center max-w-6xl mx-auto">
                ...
                <div class="relative rounded-[2.5rem] p-[2px] shadow-[0_20px_50px_-12px_rgba(79,70,229,0.4)] transform lg:scale-105 z-10 overflow-hidden group">
                    ...
                    <div class="relative bg-white h-full w-full rounded-[2.4rem] p-10 z-10 flex flex-col">
                        ...
                        <div class="relative z-10 mt-4">
                            ...
                            @else
                                <form method="POST" action="{{ $proPlan ? route('user.subscription.subscribe', $proPlan->id) : '#' }}" id="proUpgradeForm">
                                    @csrf
                ...
                {{-- ENTERPRISE TIER --}}
                <div class="bg-white/80 backdrop-blur-md border border-slate-200 rounded-[2.5rem] p-10 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 relative overflow-hidden group">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-amber-100/50 rounded-full blur-3xl group-hover:bg-amber-200/50 transition-colors z-0"></div>
                    <div class="relative z-10">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-2xl font-black text-slate-900 tracking-tight">Enterprise</h3>
                            <span class="px-2.5 py-1 bg-amber-50 text-amber-700 rounded border border-amber-200 text-[8px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1.5"><i class="fa-solid fa-crown text-amber-500"></i> Ultimate</span>
                        </div>
                        <p class="text-sm font-medium text-slate-500 h-10 leading-relaxed">Dedicated API architecture and raw data access.</p>

                        <div class="my-8 border-b border-slate-100 pb-8 flex items-end h-16">
                            <span id="enterprisePrice" data-monthly="499" data-yearly="4999" class="text-5xl font-black text-slate-900 tracking-tighter transition-all">₹499</span>
                            <span id="enterpriseBillingText" class="text-slate-400 font-bold text-sm ml-1 mb-1.5 transition-all">/mo</span>
                        </div>

                        @php $premiumPlan = $plans->firstWhere('slug', 'premium'); @endphp
                        @if($currentPlan === 'premium')
                            <button disabled class="w-full py-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 font-black uppercase tracking-widest text-[11px] cursor-not-allowed flex items-center justify-center gap-2 shadow-inner">
                                <i class="fa-solid fa-crown"></i> Active Enterprise Node
                            </button>
                        @else
                            <form method="POST" action="{{ $premiumPlan ? route('user.subscription.subscribe', $premiumPlan->id) : '#' }}" id="enterpriseUpgradeForm">
                                @csrf
                                <button type="submit" onclick="simulateStripeCheckout(event, this)" onmouseenter="audioEngine.playHover()" class="w-full py-4 rounded-xl bg-slate-900 text-white font-black uppercase tracking-widest text-[11px] hover:bg-amber-600 shadow-md hover:shadow-xl hover:shadow-amber-500/30 transition-all flex items-center justify-center gap-2 hover:-translate-y-0.5 focus:outline-none relative overflow-hidden">
                                    <div class="absolute inset-0 bg-white/20 w-full animate-[shimmer_2s_infinite] translate-x-[-100%]" style="background-image: linear-gradient(90deg, transparent, rgba(255,255,255,0.4), transparent);"></div>
                                    <span class="relative z-10"><i class="fa-solid fa-rocket mr-1"></i> Initialize Enterprise</span>
                                </button>
                            </form>
                        @endif
                        ...
                        <ul class="mt-8 space-y-4 text-sm font-medium text-slate-700">
                            <li class="flex items-start gap-3">
                                <div class="w-5 h-5 rounded-md bg-amber-50 text-amber-600 flex items-center justify-center mt-0.5 shrink-0 border border-amber-100"><i class="fa-solid fa-check text-[10px]"></i></div>
                                <span>Everything in Pro Advisor</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <div class="w-5 h-5 rounded-md bg-amber-50 text-amber-600 flex items-center justify-center mt-0.5 shrink-0 border border-amber-100"><i class="fa-solid fa-server text-[10px]"></i></div>
                                <span class="font-bold text-slate-900">Dedicated Database Instance</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <div class="w-5 h-5 rounded-md bg-amber-50 text-amber-600 flex items-center justify-center mt-0.5 shrink-0 border border-amber-100"><i class="fa-solid fa-code text-[10px]"></i></div>
                                <span class="font-bold text-slate-900">RESTful API Access Key</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <div class="w-5 h-5 rounded-md bg-amber-50 text-amber-600 flex items-center justify-center mt-0.5 shrink-0 border border-amber-100"><i class="fa-solid fa-infinity text-[10px]"></i></div>
    ...
    <div id="cancelModalContent" class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden transform scale-95 transition-transform duration-300 border border-slate-200 relative">
        ...
        <div class="p-10 text-center">
            ...
            <div class="flex gap-4">
                ...
                <form action="{{ route('user.subscription.cancel') }}" method="POST" class="flex-1" onsubmit="audioEngine.playClick()">
                    @csrf @method('DELETE')
