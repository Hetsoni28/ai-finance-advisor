...
@props([
    ...
    'icon'       => null,
    'value'      => 0,
    'goal'       => null,          // 🔥 NEW: Triggers a glowing pulse if value >= goal
    'insight'    => null,          // 🔥 NEW: Hoverable AI Insight string
...
@php
/* ================= 1. SAFE DATA SERIALIZATION & MATH ================= */
...
// Goal Computation
$hitGoal = false;
if ($goal !== null && is_numeric($value) && is_numeric($goal)) {
    $hitGoal = (float)$value >= (float)$goal;
}
...
$accents = [
    'indigo'  => ['iconBg' => 'bg-indigo-50',  'iconText' => 'text-indigo-600',  'hex' => '#4f46e5', 'glow' => 'rgba(79,70,229,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(79,70,229,0.4)]'],
    'emerald' => ['iconBg' => 'bg-emerald-50', 'iconText' => 'text-emerald-600', 'hex' => '#10b981', 'glow' => 'rgba(16,185,129,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(16,185,129,0.4)]'],
    'rose'    => ['iconBg' => 'bg-rose-50',    'iconText' => 'text-rose-600',    'hex' => '#f43f5e', 'glow' => 'rgba(244,63,94,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(244,63,94,0.4)]'],
    'sky'     => ['iconBg' => 'bg-sky-50',     'iconText' => 'text-sky-600',     'hex' => '#0ea5e9', 'glow' => 'rgba(14,165,233,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(14,165,233,0.4)]'],
    'amber'   => ['iconBg' => 'bg-amber-50',   'iconText' => 'text-amber-600',   'hex' => '#f59e0b', 'glow' => 'rgba(245,158,11,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(245,158,11,0.4)]'],
    'slate'   => ['iconBg' => 'bg-slate-100',  'iconText' => 'text-slate-600',   'hex' => '#475569', 'glow' => 'rgba(71,85,105,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(71,85,105,0.4)]'],
    'fuchsia' => ['iconBg' => 'bg-fuchsia-50', 'iconText' => 'text-fuchsia-600', 'hex' => '#d946ef', 'glow' => 'rgba(217,70,239,0.15)', 'pulse' => 'shadow-[0_0_0_0_rgba(217,70,239,0.4)]'],
];
...
{{-- ================= THE COMPONENT HTML ================= --}}
<div class="relative w-full perspective-[2000px] group/widget z-10 hover:z-20 min-h-[220px]" 
     x-data="kpiWidgetEngine({ 
    ...
    <div x-ref="cardBody" 
         class="relative w-full h-full transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] transform-style-3d {{ $hitGoal ? 'animate-kpi-pulse ' . $activeAccent['pulse'] : '' }} {{ $hover ? 'group-hover/widget:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)]' : '' }} rounded-[2.5rem]"
         :class="isFlipped ? 'rotate-y-180' : ''">

        {{-- ----------------------------------------------------------------- --}}
        {{-- FRONT FACE OF THE CARD                                            --}}
        {{-- ----------------------------------------------------------------- --}}
        <div class="absolute inset-0 w-full h-full backface-hidden flex flex-col rounded-[2.5rem] overflow-hidden {{ $baseClass }}">
            ...
            <div class="absolute inset-0 pointer-events-none z-0">
                ...
                <div class="absolute -right-10 -top-10 w-48 h-48 rounded-full blur-[60px] opacity-60 transition-transform duration-1000 group-hover/widget:scale-[1.8]" style="background-color: {{ $activeAccent['glow'] }};"></div>

                {{-- Interactive Glare Effect --}}
                @if($hover)
                    <div class="absolute inset-0 overflow-hidden rounded-[2.5rem]" x-ref="glareWrapper">
                ...
                @if($chartType && $safeChartData !== '[]')
                    <div class="absolute bottom-0 left-0 right-0 h-28 opacity-80 group-hover/widget:opacity-100 transition-all duration-700 translate-z-10 {{ $padding }} pt-0 pb-0">
            ...
            <div class="relative z-10 flex-1 flex flex-col {{ $padding }} h-full">
                ...
                <div class="flex justify-between items-start mb-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <h4 class="text-[11px] font-black uppercase tracking-widest text-slate-500">{{ $title }}</h4>
                                ...
                                {{-- 🔥 BEAST MODE: AI Insight Chip --}}
                                @if($insight)
                                    <div class="relative group/insight">
                                        <div class="w-5 h-5 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-500 cursor-help hover:bg-indigo-500 hover:text-white transition-colors shadow-sm">
                                            <i class="fa-solid fa-wand-magic-sparkles text-[9px] animate-pulse"></i>
                                        </div>
                                        {{-- Insight Tooltip --}}
                                        <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-slate-900 text-white p-3 rounded-xl shadow-xl z-50 text-[10px] font-bold pointer-events-none opacity-0 group-hover/insight:opacity-100 transition-opacity duration-300 border border-slate-700">
                                            <span class="text-indigo-400 uppercase tracking-widest block mb-1 font-black">AI Insight</span>
                                            {{ $insight }}
                                            <div class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-3 h-3 bg-slate-900 transform rotate-45 border-b border-r border-slate-700"></div>
                                        </div>
                                    </div>
                                @endif
                            </div>
                    ...
                    @if($menu)
                        <div class="relative translate-z-30" x-data="{ menuOpen: false }" @click.away="menuOpen = false">
                            <button @click="menuOpen = !menuOpen" @mouseenter="if(typeof window.audioEngine !== 'undefined') window.audioEngine.playHover()" class="w-8 h-8 rounded-xl flex items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-900 transition-colors focus:outline-none border border-transparent hover:border-slate-200 shadow-sm bg-white/50 backdrop-blur-sm">
                                <i class="fa-solid fa-ellipsis-vertical"></i>
                            </button>
                            
                            {{-- Dropdown (Safely escapes bounds due to decoupled overflow) --}}
                            <div x-show="menuOpen" x-cloak
                                ...
                                <button @click="menuOpen = false; if(typeof window.audioEngine !== 'undefined') window.audioEngine.playClick(); $dispatch('notify', {message: 'Syncing telemetry...', type: 'info'})" class="w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors flex items-center gap-3">
                                ...
                                <button @click="menuOpen = false; if(typeof window.audioEngine !== 'undefined') window.audioEngine.playClick(); flipCard();" class="w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors flex items-center gap-3">
                                ...
                                <button @click="menuOpen = false; if(typeof window.audioEngine !== 'undefined') window.audioEngine.playClick(); downloadCSV();" class="w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 transition-colors flex items-center gap-3">
                                    <div class="w-6 h-6 rounded bg-emerald-100/50 flex items-center justify-center"><i class="fa-solid fa-file-csv text-[10px]"></i></div> Export Dataset
                                </button>
                                @endif
                            </div>
                        </div>
                ...
                {{-- MAIN VALUE & TREND --}}
                <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mt-auto mb-[50px] translate-z-20">
                    
                    {{-- Big Number (With Robinhood Scrubbing Dynamics) --}}
                    ...
                    {{-- Trend Pill (Fades out when scrubbing chart) --}}
                    @if($trend)
        ...
        {{-- ----------------------------------------------------------------- --}}
        {{-- BACK FACE OF THE CARD (Raw Data Inspector & Exporter)             --}}
        {{-- ----------------------------------------------------------------- --}}
        <div class="absolute inset-0 w-full h-full backface-hidden rotate-y-180 bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-2xl overflow-hidden flex flex-col pointer-events-none" :class="isFlipped ? 'pointer-events-auto' : ''">
            ...
            <div class="absolute -left-20 -top-20 w-64 h-64 rounded-full blur-3xl pointer-events-none opacity-20" style="background-color: {{ $activeAccent['hex'] }};"></div>
            
            <div class="flex justify-between items-center p-6 border-b border-slate-800 relative z-10 bg-slate-900/50 backdrop-blur-md">
                ...
                <button @click="isFlipped = false; if(typeof window.audioEngine !== 'undefined') window.audioEngine.playClick();" class="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-800 text-slate-400 hover:text-white border border-slate-700 transition-colors focus:outline-none shadow-sm">
            ...
            <div class="flex-1 p-6 relative z-10 flex flex-col justify-center space-y-4">
                <div class="flex justify-between items-center border-b border-slate-800 pb-2">
                    ...
                    <span class="text-xs font-mono text-slate-300" x-text="'WID-' + btoa('{{ $title }}').substring(0,8).toUpperCase()"></span>
                </div>
                <div class="flex justify-between items-center border-b border-slate-800 pb-2">
                    <span class="text-xs font-bold text-slate-500">Final Vector Value</span>
                    <span class="text-xs font-mono text-emerald-400">{{ $displayValue }}</span>
                </div>
                <div class="flex justify-between items-center border-b border-slate-800 pb-2">
                    <span class="text-xs font-bold text-slate-500">Dataset Length</span>
                    <span class="text-xs font-mono text-indigo-400" x-text="chartData ? chartData.length + ' points' : 'Null'"></span>
                </div>
                <div class="flex justify-between items-center border-b border-slate-800 pb-2">
                    <span class="text-xs font-bold text-slate-500">Data Variance (σ²)</span>
                    <span class="text-xs font-mono text-rose-400" x-text="calculateVariance()"></span>
                </div>
                
                @if($chartType && $safeChartData !== '[]')
                <button @click="downloadCSV()" class="w-full mt-2 py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-[0_4px_15px_rgba(79,70,229,0.3)] flex items-center justify-center gap-2 focus:outline-none">
                    <i class="fa-solid fa-download"></i> Extract CSV Payload
                </button>
                @endif
            </div>
        </div>
...
@push('styles')
<style>
    /* 3D Hardware Acceleration Classes */
    .perspective-\[2000px\] { perspective: 2000px; }
    .transform-style-3d { transform-style: preserve-3d; }
    .backface-hidden { backface-visibility: hidden; -webkit-backface-visibility: hidden; }
    .rotate-y-180 { transform: rotateY(180deg); }
    
    /* Z-Depth Parallax Classes */
        ...
        Alpine.data('kpiWidgetEngine', (config) => ({
            ...
            init() {
                ...
                if (this.chartType && this.chartType !== 'none' && Array.isArray(this.chartData) && this.chartData.length > 0) {
                    // Small timeout ensures the DOM has painted the canvas size correctly
            ...
            calculateVariance() {
                ...
                const mean = this.chartData.reduce((a,b) => a+b, 0) / this.chartData.length;
                const variance = this.chartData.reduce((a,b) => a + Math.pow(b - mean, 2), 0) / this.chartData.length;
                // Return Standard Deviation relative to mean as a pseudo-variance metric
                const stdDev = Math.sqrt(variance);
                const variancePercent = (stdDev / (mean || 1)) * 100;
                return '±' + variancePercent.toFixed(1) + '%';
            },
            ...
            downloadCSV() {
                ...
                link.setAttribute("download", `${this.metricName.replace(/\s+/g, '_')}_export.csv`);
                document.body.appendChild(link); 
                link.click();
                document.body.removeChild(link);

                this.$dispatch('notify', { message: 'Dataset extracted successfully.', type: 'success' });
            },
            ...
            // ----------------------------------------------------
            // 3D MOUSE INTERACTION ENGINE (Hardware Accelerated)
            // ----------------------------------------------------
            handleTilt(e) {
                if(this.isFlipped) return; // Freeze tilt if inspecting data
                ...
                // Subtle tilt limits (4 degrees max)
                ...
                // Move Glare Overlay
                if(this.$refs.glare) {
                    this.$refs.glare.style.opacity = '0.4';
                    // Center the 600x600 glare orb exactly on the cursor
            ...
            // ----------------------------------------------------
            // CHART.JS FACTORY (ROBINHOOD CROSSHAIR & SYNC)
            // ----------------------------------------------------
            initChart() {
                ...
                if(this.chartType === 'line') {
                    // Create fading gradient fill
                    gradient = ctx.createLinearGradient(0, 0, 0, 112); // Height roughly 28 tailwind units
                ...
                const labels = this.chartData.map((_, i) => `Point ${i}`);

                // 🔥 BEAST MODE: Robinhood Crosshair Plugin
                ...
                const commonOptions = {
                    ...
                    plugins: { 
                        ...
                        tooltip: { 
                            enabled: false, // Disable native tooltip, we update the HTML directly
                            external: function(context) {
                                // Haptic Audio Tick on scrub
                                if(context.tooltip.opacity !== 0 && typeof window.audioEngine !== 'undefined') {
                                    if(!self.lastTick || Date.now() - self.lastTick > 75) {
                    ...
                    scales: {
                        ...
                        y: { 
                            display: false, 
                            min: Math.min(...this.chartData) * 0.85, // Extra bottom padding
                            max: Math.max(...this.chartData) * 1.15  // Extra top padding
                        }
                    },
                    interaction: { intersect: false, mode: 'index' },
                    
                    // 🔥 Magic Sauce: Live sync chart hover with the big HTML number
                ...
                const lineConfig = {
                    ...
                    plugins: [crosshairPlugin] // Inject crosshair
                };
