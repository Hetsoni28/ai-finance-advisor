...
class FamilyInviteService
{
    ...
    public function sendEmailInvite(Family $family, string $email): FamilyInvite
    {
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        $isAlreadyMember = DB::table('family_members')
        ...
        */
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('SMTP Dispatch Failed: ' . $e->getMessage() . ' - Continuing without sending email.');
            // We intentionally swallow the exception and let the process complete instead of throwing so that the user can still create the invite.
            session()->flash('warning', 'Invite created, but SMTP failed to dispatch the email.');
        }
